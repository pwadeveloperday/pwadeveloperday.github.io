...
categories: 
  - "PWA"
  - "WebAlmanac"
coverImage: "/images/blog/20221003/hero_lg.avif"
coverWidth: 16
coverHeight: 9
...
注：作为 Web 技术的集合，PWA 并不与 Web 平台的其余部分隔绝。前面的章节专门介绍了[能力](https://almanac.httparchive.org/zh-CN/2022/capabilities)，今年我们也研究了在 PWA 中使用这些新的高级功能。
...
为了比较，我们进行了与去年类似的调查，使我们能够了解 Service Worker 使用的演变。去年的章节解释了[为什么找出 Service Worker 的实际使用情况并非易事](https://almanac.httparchive.org/en/2021/pwa#service-workers-usage)，今年同样如此。
...
![支持 Service Worker 的网站（按排名）](/images/blog/20221003/1.png)
图1：支持 Service Worker 的网站（按排名）

在前 1000 个网站中，支持 Service Worker 的页面数量没有明显的变化，桌面端略有下降，而移动端的增长则更小，但所有其他类别都有所增加。如果我们遵循[去年的推理](https://almanac.httparchive.org/zh-CN/2021/pwa#fig-2)，即假设大型网站更快地采用先进技术，那么看到其他类别的更多增长是合理的。似乎较小的公司和开发人员已经学习并采用了从流量更多的公司案例研究和示例中分享的技术。
...
![最常用的 Service Worker 事件](/images/blog/20221003/2.png)

图 2：最常用的 Service Worker 事件
...
![移动端 Service Worker 的 `install` 事件使用率](/images/blog/20221003/3.png)

图 3：移动端 Service Worker 的 `install` 事件使用率
...
![桌面端 Service Worker 的 `notificationclick` 事件使用率](/images/blog/20221003/4.png)

图 4：桌面端 Service Worker 的 `notificationclick` 事件使用率
...
![桌面端 Service Worker 的 `fetch` 事件](/images/blog/20221003/5.png)

图 5：桌面端 Service Worker 的 `fetch` 事件
...
![skipWaiting()` 方法的使用](/images/blog/20221003/6.png)

图 6：skipWaiting()` 方法的使用

类似于[去年的数据](https://almanac.httparchive.org/zh-CN/2021/pwa#other-popular-service-worker-features)，用于立即激活 Service Worker 的 [`skipWaiting`](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerGlobalScope/skipWaiting) 方法在开发人员中仍然非常流行，存在于 60% 的桌面和 59% 的移动 Web 应用中。

这些是最常用的 Service Worker 对象：

![最常用的 Service Worker 对象](/images/blog/20221003/7.png)

图 7：最常用的 Service Worker 对象

这些是最常用的对象方法：

![最常用的 Service Worker 对象方法](/images/blog/20221003/8.png)

图 8：最常用的 Service Worker 对象方法
...
![桌面端的 manifest 文件的百分比](/images/blog/20221003/9.png)

图 9：桌面端的 manifest 文件的百分比
...
![Service Worker 及 Manifest 的使用率](/images/blog/20221003/10.png)

图 10：Service Worker 及 Manifest 的使用率
...
![常用 PWA manifest 成员](/images/blog/20221003/11.png)

图 11：常用 PWA manifest 成员
...
![PWA manifest display 值](/images/blog/20221003/12.png)

图 12：PWA manifest display 值

`display:standalone` 是最常见的 `display` 模式，几乎 3/4 定义了 `display` 模式的网站都使用这个值。它也是使应用可安装的 `display` 模式之一。

#### `icons` 成员

![顶级 PWA 的 manifest 图标尺寸](/images/blog/20221003/13.png)

图 13：顶级 PWA 的 manifest 图标尺寸
...
![顶级 PWA 的 manifest 类别](/images/blog/20221003/14.png)

图 14：顶级 PWA 的 manifest 类别

除了使用 Service Worker 来定义的 “PWA 网站” 之外，下面是所有网站的数据：

![顶级 manifest 类别](/images/blog/20221003/15.png)
...
![移动端带有 `related_applications` 字段的 mainfest 文件](/images/blog/20221003/16.png)

图 16：移动端带有 `related_applications` 字段的 mainfest 文件
...
![大多数使用的 Fugu API（桌面端）](/images/blog/20221003/17.png)

图 17：大多数使用的 Fugu API（桌面端）
...
由于有单独的章节介绍[能力](https://almanac.httparchive.org/zh-CN/2022/capabilities)，因此在此我们不作过多叙述。
...
![桌面端 Lighthouse PWA 审计](/images/blog/20221003/19.png)

图 19：桌面端 Lighthouse PWA 审计
...
![移动端 Lighthouse PWA 审计](/images/blog/20221003/20.png)

图 20：移动端 Lighthouse PWA 审计
...
![桌面 Lighthouse 得分](/images/blog/20221003/21.png)

图 21：桌面 Lighthouse 得分

![移动端 Lighthouse 得分](/images/blog/20221003/22.png)

图 22：移动端 Lighthouse 得分
...
![PWA 页面的 Workbox 使用率](/images/blog/20221003/23.png)

图 23：PWA 页面的 Workbox 使用率
...
![Top Workbox packages](/images/blog/20221003/24.png)
...
![通知接受率](/images/blog/20221003/25.png)
...
- 阅读原文: https://almanac.httparchive.org/zh-CN/2022/pwa
